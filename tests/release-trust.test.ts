...
describe("release trust checks", () => {
  ...
  it("documents heuristic/static limits explicitly in README", async () => {
    ...
    expect(readme).toContain("Security and auth findings are heuristic signals, not guarantees.");
    expect(readme).toContain("No warning does not mean safe.");
    expect(readme).toContain("Do not use ForgeLens as a security scanner or compliance gate.");
  });
});
