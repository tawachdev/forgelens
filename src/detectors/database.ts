...
export async function detectDatabase(root: string, outDir: string): Promise<DatabaseInfo> {
  ...
  const [
    ...
    fg(["supabase/**/*", "**/*supabase*.@(ts|tsx|js|jsx)"], { cwd: root, ignore }),
    fg(["**/*.sql"], { cwd: root, ignore }),
    fg(["**/migrations/**/*", "**/migration/**/*", "**/migrate/**/*"], { cwd: root, ignore }),
    fg(["**/*db*.@(ts|tsx|js|jsx)", "**/*database*.@(ts|tsx|js|jsx)", "**/lib/*sql*.@(ts|tsx|js|jsx)"], {
      cwd: root,
      ignore
    }),
  ...
  if (sqlFiles.length > 0 || migrationFiles.length > 0) {
    providers.push({
      ...
      evidenceFiles: uniqueSorted([...sqlFiles, ...migrationFiles]).slice(0, 12),
