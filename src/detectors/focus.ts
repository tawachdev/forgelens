...
export function buildFocusMap(report: FocusInput): FocusItem[] {
  ...
  const adminRouteFiles = report.routes
    .filter(
      (route) => route.route === "/admin" || route.route.includes("/admin/"),
    )
    .map((route) => route.file);
  ...
  pushItem(items, {
    ...
    reason:
      "Review login, roles, and access boundaries before changing protected behavior.",
    files: uniqueSorted([
  ...
  pushItem(items, {
    ...
    area: "Server actions",
    reason:
      "Write actions can change data; verify auth, ownership, and input validation.",
  ...
  pushItem(items, {
    ...
    reason:
      "Missing examples or risky public env names can create production config mistakes.",
    files: uniqueSorted([
      ...report.env.envFiles,
      ...report.env.serverSecretClientFiles,
    ]),
  });
  ...
  pushItem(items, {
    ...
    reason:
      "Pages, forms, and accessibility-risk files deserve review during user-facing changes.",
    files: uniqueSorted([
  ...
  pushItem(items, {
    ...
    reason:
      "Large files, client components, fetch calls, and external APIs can slow or break flows.",
    files: uniqueSorted([
...
export function buildFocusFileScores(report: FocusInput): FocusFileScore[] {
  ...
  addScores(
    ...
    report.env.serverSecretClientFiles,
    50,
    "server secret-like env in client file",
  );
  ...
  addScores(
    ...
    report.performance.uncachedFetchFiles,
    12,
    "fetch without nearby cache hint",
  );
  addScores(
    scores,
    report.performance.externalApiFiles,
  ...
  addScores(
    scores,
    report.security.sensitiveFiles,
    12,
